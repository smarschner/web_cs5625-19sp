...
sub do_homeworks {
	...
	if (&compare_date($prevm, ($prevd+1), $dm, $dd) < 0 &&
	    &compare_date($dm, $dd, $month, ($day+1)) <= 0) {
	    if ($result ne '') { $result .= ', '; }
	...
	if (&compare_date($prevm, ($prevd+1), $om, $od) < 0 &&
	    &compare_date($om, $od, $month, ($day+1)) <= 0) {
	    if ($result ne '') { $result .= ', '; }
...
'<table class=sched cellpadding="0" cellspacing="0">
  <tr>
    ...
    <th class=topic>topic</td>
    <th class=reading>reading</td>
...
while ($month < $cfg{'end_month'} || $day < $cfg{'end_day'}) {
    ...
    my $odd_week = ($ni % 4 == 0 || $ni % 4 == 1);  # TODO needs to be adjusted depending on calendar
    if ($holiday ne '') {
	if ($odd_week) {
	...
	my $reading = $readings[$mi];
	$reading =~ s/\&/\&amp;/g;
	$reading = &fix_blank($reading);
	if (!($reading =~ /^</)) {
	    $reading =~ s/--/\&ndash;/;
	}
